...
function(      WebGL,         glMatrix,      Camera )
{
	...
	function RenderCanvas3D(isBlendModeOne, spriteType = "", entityType = "")
	{
		// Nothing to render ?
		...
		const shouldRenderShadow = spriteType !== "shadow" && (spriteType === "body" || spriteType === "head")
			&& (entityType === 5 || entityType === 6 || entityType === 0);
		if (shouldRenderShadow) {
			...
			this.offset[0] += 1.5; // Offset shadow slightly
			this.offset[1] += 1.5;
